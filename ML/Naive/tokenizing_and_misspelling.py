# -*- coding: utf-8 -*-
"""
Created on Thu May  5 14:31:35 2016
...
def fixList(positive_tweets):  
    for i in range(len(positive_tweets)):
        positive_tweets[i] = re.sub(r'\w+:\/{2}[\d\w-]+(\.[\d\w-]+)*(?:(?:\/[^\s/]*))*', '', positive_tweets[i])
        positive_tweets[i] = re.sub('(?<!\w)@[\w]+', '', positive_tweets[i])
        positive_tweets[i] = re.sub("[^a-zA-Z]", " ", positive_tweets[i])
        positive_tweets[i] = positive_tweets[i].lower()
        positive_tweets[i] = re.sub(' +', ' ', positive_tweets[i])
        positive_tweets[i] = positive_tweets[i].split(' ')
    
    for i in range(len(positive_tweets)):
        for z in range(len(positive_tweets[i])):
            if(positive_tweets[i][z] in _misspelling_dict):
                positive_tweets[i][z] = _misspelling_dict[positive_tweets[i][z]][0]
                positive_tweets[i][z] = re.sub("[^a-zA-Z]", "", positive_tweets[i][z])
...
#to use this code, first run loadDataset and gather your lists.  Then run them through fixList, and they will then be returned tokenized